...
def resolve_rpath(binary_path, rpath_lib):
    try:
        ...
        # Extract all RPATH entries from the binary
        for i, line in enumerate(output.splitlines()):
            if "cmd LC_RPATH" in line:
                # Look ahead a few lines for the path
                for j in range(1, 5):
                    if i+j < len(output.splitlines()) and "path" in output.splitlines()[i+j]:
                        path_line = output.splitlines()[i+j]
                        rpath = path_line.strip().split("path ")[1].strip()
        ...
        # For synfig and mlt libraries specifically
        special_paths = [
